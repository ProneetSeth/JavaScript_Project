...
$(".input-cell").mousemove(function (e) {
    ...
    if (e.buttons == 1) {
        //$(".input-cell.selected").removeClass("selected top-selected bottom-selected left-selected right-selected");
        if (e.pageX > ($(window).width() - 100) && !scrollXRStarted) { /*jab current window ke end pe ate hain toh scrollXR() trigger hota hai and scrollXRStrated start hota hai*/
            scrollXR();
        }
...
/* If we keep on dragging to the right by pressing left key, it should select the cell till the area we dragged */
/* 1st How to know the width/max size of a window ?
console.log($(window).width());  <=== JQuery
console.log(e.pageX);  <=== Will give the max width on the X direction */

/*Now will apply the scroll on dragging */

let scrollXRInterval;
function scrollXR() {  /* This function will take to the end of the window by using the interval continuously */
...
/* to stop in the middle while scrolling */
$(".input-cell").mouseup(function (e) {
    clearInterval(scrollXRInterval); /*stop the interval of scrolling */
    scrollXRStarted = false; /* when stop dragging with left click, it will stop scrolling */
})